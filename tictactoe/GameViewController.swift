...
class GameViewController: UIViewController {
    ...
    // load view with player name and boxes
    override func viewDidLoad() {
        super.viewDidLoad()
        
        playerNameLabel.text = playerName.lowercased() + ":"
    ...
    // show X or O when user taps
    func createTap(on imageView: UIImageView, type box: Box) {
        let tap = UITapGestureRecognizer(target: self, action:
            #selector(self.boxClicked(_:)))
        tap.name = box.rawValue
        imageView.isUserInteractionEnabled = true
        imageView.addGestureRecognizer(tap)
    }
    
    // function for what occurs if box clicked, checks if winner
    ...
    // decide whether X or O image should show in box
    func makeChoice(_ selectedBox: UIImageView) {
        guard selectedBox.image == nil else { return }
        if lastValue == "x" {
            selectedBox.image = #imageLiteral(resourceName: "Image-2")
            lastValue = "o"
        } else {
            selectedBox.image = #imageLiteral(resourceName: "Image")
            lastValue = "x"
        }

    }
    
    // all possibilities for winning game
    func checkIfWon() {
        var correct = [[Box]]()
    ...
    // determine which location user/computer selected
    ...
    // function for once game is exited
    @IBAction func closedButtonClicked(_ sender: UIButton) {
        let alert = UIAlertController(title: "Game Over", message: "Thanks for playing.", preferredStyle: UIAlertController.Style.alert)
